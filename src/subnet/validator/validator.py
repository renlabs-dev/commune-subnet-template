...
def set_weights(
    ...
    score_dict = cut_to_max_allowed_weights(score_dict)
...
def cut_to_max_allowed_weights(
    score_dict: dict[int, float], max_allowed_weights=420
) -> dict[int, float]:
    """
...
def get_subnet_netuid(clinet: CommuneClient, subnet_name: str = "foo"):
    """
    Retrieves the network UID of the

    Args:
        client (CommuneClient): The CommuneX client.
        subnet_name (str, optional): The name of the subnet.
...
class TextValidator(Module):
    ...
    This class retrieves module addresses from a subnet, prompts the modules to generate
    answers to a given question, and scores the generated answers against the validator's
    own answers.

    Attributes:
        client (CommuneClient): The CommuneClient instance used to interact with the subnet.
        key (Keypair): The keypair used for authentication.
        netuid (int): The unique identifier of the subnet.
        val_model (str): The validation model used for scoring answers.
        call_timeout (int): The timeout value for module calls in seconds (default: 60).

    Methods:
        get_modules(client: CommuneClient, netuid: int) -> dict[int, str]:
            Retrieves all module addresses from the subnet.

        _get_miner_prediction(question: str, miner_info: tuple[list[str], Ss58Address]) -> str | None:
            Prompts a miner module to generate an answer to the given question.

        _score_miner(miner_answer: str | None) -> float:
            Scores the generated answer against the validator's own answer.

        get_miner_prompt() -> str:
            Generates a prompt for the miner modules.

        validate_step(syntia_netuid: int) -> None:
            Performs a validation step by generating questions, prompting modules, and scoring answers.

        validation_loop(settings: ValidatorSettings) -> None:
            Runs the validation loop continuously based on the provided settings.
    """
    ...
    def _score_miner(self, miner_answer: str | None) -> float:
        """
        Replace with your custom validation logic
        """
    ...
    async def validate_step(self, syntia_netuid: int) -> None:
        ...
        Args:
            ...
            syntia_netuid: The netuid of the subnet.
        """
    ...
    def validation_loop(self, settings: ValidatorSettings) -> None:
        ...
        while True:
            ...
            _ = asyncio.run(self.validate_step(self.netuid))
